...
### Security
- **Unsharing a page did not revoke bridge access, and nothing said so.** The bridge's access never came from VS Code's sharing model: it attaches over CDP to every `BrowserTab` in the window, and the proposed `browser` API exposes no sharing state at all, so share/unshare is structurally invisible to it. Two changes: `/status` now reports `sharing: { enforced, mode, note }` stating plainly that unsharing does not detach and naming the real revocation paths; and the new opt-in `integratedBrowserMcp.enforceSharing` restricts the bridge to tabs it opened itself — adopted user tabs are detached and later calls on their `tabId` fail, while `browser_tab_open` / `browser_navigate` keep working. (URL-matching against VS Code's shared-page listing was considered and dropped: a URL identifies content, not a principal, so a page could navigate itself to a "shared" URL to acquire access.)
...
- **`browser_tab_open` no longer returns stale metadata.** It answered immediately after `Page.navigate`, reporting `url: "about:blank"` and a title of `"about:blank (about:blank)"` for a page that loaded correctly a moment later. `openTab` now awaits `settleNavigation()`, and the `title` getter prefers the live `document.title` while VS Code is still seeding `BrowserTab.title` with the page URL — the cause of `pages_discover`, `browser_tab_list`, and the real document title disagreeing three ways.
- **The degraded (no-proposal) path is now loud instead of silent.** It reported `lmPageDiscovery: { available: true }` and otherwise looked healthy while lacking attachment, `browser_tab_open`, and clean titles, so a misconfiguration read as a pile of extension bugs. `/status` now returns `degraded: true` with a `warning` naming cause *and* remedy, the same text is attached to no-attached-page errors and to discovery's `hint`, and discovery explicitly flags that VS Code's "can be interacted with" copy is not true on that path.
...
- **`browser_tab_close` now actually closes the tab.** On the websocket/debug-session path it only tore down the CDP connection: `tab.browserTab` is null there, so the close branch never ran and the browser editor stayed open in VS Code despite the call reporting `{ closed: … }` and the bridge dropping to `tabCount: 0`. This is why `list_browser_pages`' unshared count never decremented — a failed close, not a stale count. The debug session backing the tab is now terminated, which closes the editor.
...
- `/status` now reports `capabilities: { tabOpen, attachExistingPages, multiTab, reason? }` and `attachablePages`, so an agent can tell what this build supports *before* a tool fails, and doesn't read `cdp: disconnected, tabCount: 0` as "nothing to act on" while a discovered, reachable page exists.
